...
def get_gcc_warning(path="path", line=1, col=1, text="text", flag="flag"):
    fmt = "{0}:{1}:{2}: warning: {3} [-W{4}]\n"
    return fmt.format(path, line, col, text, flag)


class TestGccWarningParser():
    def test_empty(self):
        warnings = ""
        parser = GccWarningParser()
        results = parser.load(BytesIO(b(warnings)))
        assert len(results) == 0

    def test_one_error(self):
        warnings = get_gcc_warning(
        ...
        assert diag.tool_info.name == "gcc"
