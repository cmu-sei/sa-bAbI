...
def get_clang_warning(path="path", line=1, col=1, text="text", flag="flag"):
    fmt = "{0}:{1}:{2}: warning: {3} [-W{4}]\n"
    return fmt.format(path, line, col, text, flag)


class TestClangWarningParser():
    def test_empty(self):
        warnings = b""
        parser = ClangWarningParser()
        results = parser.load(BytesIO(warnings))
        assert len(results) == 0

    def test_one_error(self):
        warnings = get_clang_warning(
            path="/test/path", line=1, col=1, text="testtext", flag="testflag")
        parser = ClangWarningParser()
        results = parser.load(BytesIO(b(warnings)))
        assert len(results) == 1
        diag = results[0]

        assert diag.tool_info.name == "clang"
        assert diag.kind == "-Wtestflag"
    ...
    def test_multiple_diags(self):
        warnings = "".join([
            get_clang_warning(
                ...
                col=i,
                text="testtext" + str(i),
                flag="testflag" + str(i)) for i in range(10)
        ])
        parser = ClangWarningParser()
        results = parser.load(BytesIO(b(warnings)))
        ...
        for i in range(10):
            diag = results[i]
            assert diag.tool_info.name == "clang"
            assert diag.kind == "-Wtestflag" + str(i)
