...
def preprocess(infile, outfile, includes, defines):
    include_args = ["-I" + inc for inc in includes]
    define_args = ["-D" + dfn for dfn in defines]
    cmd = ["clang", "-E"] + include_args + define_args + [infile]
...
if __name__ == "__main__":
    ...
    for item in args.files:
        root,base = os.path.split(item)
        outfile = os.path.join(args.outdir, base)
        preprocess(item, outfile, args.include, args.define) 
