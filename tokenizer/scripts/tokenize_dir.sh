...
ls $src_dir | while read cfile; do
    cfile_path=$src_dir/$cfile
    preproc_file=/tmp/$cfile
    clang -E $cfile_path > $preproc_file
    tokenize $preproc_file | filter_tokens.py > $out_dir/$cfile.tok
done
